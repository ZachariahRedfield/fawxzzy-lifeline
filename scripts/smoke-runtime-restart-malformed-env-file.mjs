...
const appName = `runtime-smoke-restart-malformed-env-file-${uniqueSuffix}`;
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(path.join(tmpdir(), "lifeline-runtime-restart-malformed-env-file-smoke-"));
...
try {
  ...
  if (restartResult.code === 0) {
    throw new Error(
      `Expected restart to fail when env.file is malformed.\nstdout:\n${restartResult.stdout}\nstderr:\n${restartResult.stderr}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected restart failure to clearly explain malformed env.file contents.\nstdout:\n${restartResult.stdout}\nstderr:\n${restartResult.stderr}`,
    );
  }
