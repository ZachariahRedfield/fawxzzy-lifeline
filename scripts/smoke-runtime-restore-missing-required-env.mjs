...
const appName = `runtime-smoke-restore-missing-required-env-${uniqueSuffix}`;
const runtimePort = 9300 + Math.floor(Math.random() * 500);
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(path.join(tmpdir(), "lifeline-runtime-restore-missing-required-env-smoke-"));
  ...
  const manifestForRestoreMissingRequiredEnv = manifestRaw
  ...
  await writeFile(tempManifestPath, manifestForRestoreMissingRequiredEnv, "utf8");
...
try {
  ...
  delete restoreEnv[missingKey];

  const restoreResult = await run(["restore"], {
    allowFailure: true,
    env: restoreEnv,
  });
  if (restoreResult.code === 0) {
    throw new Error(
      `Expected restore to fail when required env key is no longer satisfied.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected restore failure to clearly explain missing required env key ${missingKey}.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }
