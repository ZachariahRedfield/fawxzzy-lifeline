...
const appName = `runtime-smoke-up-missing-env-file-${uniqueSuffix}`;
...
try {
  ...
  if (upResult.code === 0) {
    throw new Error(
      `Expected up to fail when env.file is missing.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }
  ...
  if (
    ...
    !combinedOutput.includes(missingEnvFile)
  ) {
    throw new Error(
      `Expected up failure to clearly explain missing env.file path.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }
