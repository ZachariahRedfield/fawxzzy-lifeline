...
const appName = `runtime-smoke-up-invalid-playbook-export-${uniqueSuffix}`;
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(
    path.join(tmpdir(), "lifeline-runtime-up-invalid-playbook-export-smoke-"),
  );
  ...
  const manifestForSmoke = manifestRaw
    .replace(/^name: .*$/m, `name: ${appName}`)
    .replace(/^port: .*$/m, `port: ${runtimePort}`);

  await writeFile(tempManifestPath, manifestForSmoke, "utf8");
  manifestPath = tempManifestPath;

  const tempPlaybookPath = path.join(tempRootDir, "playbook-export");
  ...
  await writeFile(archetypePath, "installCommand: 42\n", "utf8");

  return tempPlaybookPath;
}
...
try {
  const invalidPlaybookPath = await prepareFixtureConfig();
  await cleanup();

  const upResult = await run(
    ["up", manifestPath, "--playbook-path", invalidPlaybookPath],
    { allowFailure: true },
  );
  if (upResult.code === 0) {
    throw new Error(
      `Expected up to fail when Playbook export shape is invalid.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }
  ...
  if (!combinedOutput.includes("Playbook export shape is invalid")) {
    throw new Error(
      `Expected up failure to clearly explain Playbook export validation issue.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }

  if (
    combinedOutput.includes(`Installing ${appName} in`) ||
    combinedOutput.includes(`Building ${appName} in`) ||
    combinedOutput.includes(`Starting supervisor for ${appName}`)
  ) {
    throw new Error(
      `Expected up to fail before install/build/supervisor startup output when Playbook exports are invalid.\nstdout:\n${upResult.stdout}\nstderr:\n${upResult.stderr}`,
    );
  }
