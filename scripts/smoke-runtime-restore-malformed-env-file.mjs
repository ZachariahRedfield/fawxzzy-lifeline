...
const appName = `runtime-smoke-restore-malformed-env-file-${uniqueSuffix}`;
const runtimePort = 9260 + Math.floor(Math.random() * 500);
...
async function prepareFixtureConfig() {
  tempRootDir = await mkdtemp(
    path.join(tmpdir(), "lifeline-runtime-restore-malformed-env-file-smoke-"),
  );
  ...
  const manifestForRestoreMalformedEnv = manifestRaw
  ...
  await writeFile(tempManifestPath, manifestForRestoreMalformedEnv, "utf8");
...
try {
  ...
  await writeFile(resolvedEnvFile, "PORT\nGREETING=hello\n", "utf8");

  const restoreResult = await run(["restore"], { allowFailure: true });
  if (restoreResult.code === 0) {
    throw new Error(
      `Expected restore to fail when env.file is malformed.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Expected restore failure to clearly explain malformed env.file contents.\nstdout:\n${restoreResult.stdout}\nstderr:\n${restoreResult.stderr}`,
    );
  }
